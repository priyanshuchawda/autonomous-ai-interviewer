...
export function generateCandidateProfile(candidate: CandidateProfile): CandidateIntelligenceProfile {
  ...
  const weakAreas: string[] = [
    ...failedMissions.map((m) => `Day ${m.day}: ${m.title} (Failed)`),
    ...highAttemptMissions.map((m) => `Day ${m.day}: ${m.title} (${m.attempts} attempts required)`),
  ];

  // 5. Skipped / Unknown Areas
  const skippedAreas: string[] = skippedMissions.map((m) => `Day ${m.day}: ${m.title}`);
  ...
  return {
    ...
    skippedAreas,
    highAttemptTopics,
    recommendedFocusAreas: recommendedFocusAreas.slice(0, 5), // Keep small set (top 5)
  };
}
